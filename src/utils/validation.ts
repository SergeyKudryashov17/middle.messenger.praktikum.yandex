...
export function handleValidateField(fieldComponent: Block): boolean {
    ...
    const validateRule =
        (fieldComponent.children.inputComponent.getContent() as HTMLInputElement).getAttribute("validateRule") || "";

    const error = validate(validateRule, inputValue);
...
export function checkPasswordMatch(fieldPassword: Block, fieldPasswordRepeat: Block): boolean {
    const firstPassword: string = (fieldPassword.children.inputComponent.getContent() as HTMLInputElement).value;
    const secondPassword: string = (fieldPasswordRepeat.children.inputComponent.getContent() as HTMLInputElement).value;
...
export function validateForm(listFields: Block[]): boolean {
    return listFields.reduce((flag: boolean, component: Block) => {
        ...
        const status: boolean = errorTextNode?.textContent !== "" ? false : handleValidateField(component);
        return flag && status;
    }, true);
}
